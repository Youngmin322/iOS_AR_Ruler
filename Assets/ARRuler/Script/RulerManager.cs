...
public class RulerManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        cancelButton.onClick.AddListener(RemoveLastRulerObj);
    }
    ...
    public void RemoveLastRulerObj()
    {
        if (_rulerObjList.Count > 0)
        {
            RulerObjST lastRulerObj = _rulerObjList[_rulerObjList.Count - 1];
            _rulerObjList.Remove(lastRulerObj);
            Destroy(lastRulerObj.gameObject);

            if (_nowRulerObj == lastRulerObj)
            {
                if (_rulerObjList.Count > 0)
                {
                    _nowRulerObj = _rulerObjList[_rulerObjList.Count - 1];
                }
                else
                {
                    _nowRulerObj = null;
